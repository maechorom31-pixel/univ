...
const TYPE_ORDER = ['교과', '종합', '실기', '논술'];
const LINE_ORDER = ['인문', '자연·공학', '예체능'];

/**
 * 전형 유형 이름. **보드와 문서가 같은 말을 쓴다** — 교과 · 종합 · 논술 · 실기.
 *
 * 한동안 문서에서만 「학종」이라 적었다. 예년 보고서를 그렇게 받아 적은 것인데,
 * 카드에는 「종합」이라 적혀 있어 같은 것을 두 이름으로 부르게 됐다. 상담에서
 * 화면과 종이를 나란히 놓고 보는 자리라 말을 하나로 맞추는 편이 낫다.
 */
function typeLabel(app) {
  return stats.typeOf(app);
}

/**
 * 전형 칸 한 줄 — **유형을 두 번 적지 않는다.**
 *
 * 유형(`종합`)과 세부 이름(`종합(서류형)`)을 그냥 이으면 「종합 종합(서류형)」이
 * 되고, 「교과 교과(교과전형)」처럼 같은 말이 두 번 찍힌다. 세부 이름의 앞머리가
 * 유형 이름이면 떼고 괄호만 남긴다 — 「종합(서류형)」·「교과(교과전형)」.
 *
 ...
 * 딱 맞고, 세로로 죽 늘어서면 교과가 몇이고 종합이 몇인지 세지 않아도 보인다.
 */

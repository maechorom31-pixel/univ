...
def put(got, name, entry):
    ...
    둘 다 담으면 보드가 알아서 가린다. 모집단위가 한 장에만 있으면 그것으로
    잇고(홍익대 두 장은 모집단위가 하나도 안 겹친다 — 서울 43개, 세종 19개,
    교집합 0), 양쪽에 다 있으면 비운다(`board/ratio.js` 의 `rateOf`).
...
def main(argv):
    ...
    if twins:
        ...
        for name in sorted(set(twins)):
            print('   %-22s 페이지가 캠퍼스를 안 적었습니다. 보드가 모집단위로'
                  ' 가립니다' % name)
    if odd:

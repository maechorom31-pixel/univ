 ...
 *   셈    경쟁률이 지원 ÷ 모집과 맞아야 한다. 안 맞으면 그 줄의 숫자는 학과
 *           것이 아니라 전형 전체의 것이 퍼진 것이다 — 비운다.
  ...
  for (const u of doc.univs) {
    const tracks = (u.t || []).map((t) => {
      ...
      for (const r of t.r || []) {
        ...
        if (!key) continue;
        /*
         * **셈이 맞지 않는 줄은 학과별 값이 아니다.**
         *
         * 대학 페이지가 정원 외 전형이나 실기 전형을 적을 때, 모집인원과
         * 경쟁률 칸을 묶어(rowspan) 전형 전체로 한 번만 쓰는 곳이 있다.
         * 표를 펴면 그 총계가 학과마다 그대로 복사된다. 광주대
         * 「수급자ㆍ차상위」가 그랬다 — 문헌정보학과에 「모집 44 · 지원 3 ·
         * 경쟁 4.86」이 붙었는데, 44 와 4.86 은 전형 전체의 숫자이고
         * 3 만 그 학과 것이다.
         *
         * 경쟁률은 지원 ÷ 모집이므로, 셋이 서로 안 맞으면 적어도 하나는
         * 그 학과의 것이 아니다. 자료 13,134행 가운데 1,382행이 그러했고,
         * 그 줄들은 **모두** 같은 전형 안에서 모집·경쟁률이 똑같이 되풀이되는
         * 줄이었다(혼자 어긋나는 줄은 하나도 없었다). 틀린 숫자를 붙이느니
         * 비워 두는 편이 낫다 — 붙일 수 없는 줄로 적어 둔다.
         */
        const q = r[1], a = r[2], rate = r[3];
        const spread = q > 0 && a != null && rate != null
          && Math.abs(rate - a / q) > 0.011;
        rows.set(key, rows.has(key) ? null : {
          unit: r[0], quota: q, applied: a, rate, sub: r[4] || '', spread,
        });
      }
      return {
        name: t.n,

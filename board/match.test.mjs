...
console.log('쌍둥이 행 걸러내기');
{
  ...
  const R = (cat, type, dept, g70) => ['호남', '가대', 2026, cat, type, dept, '자연', 10, 5, 3.0, g70, 50];

  // 값이 전부 같은데 카테고리만 다른 쌍둥이 — 전형 이름이 진실을 말한다
  const twin = indexIpgyeol(doc([R('종합', '종합(일반학생)', '간호학과', 3.6),
    R('교과', '종합(일반학생)', '간호학과', 3.6)]));
  const kept = twin.byKey.get(key('가대', '간호학과'));
  eq(kept.length, 1, '쌍둥이는 한 줄만 남긴다');
  eq(kept[0].cat, '종합', '「종합(…)」이면 종합이다 — 이름을 믿는다');

  // 「교과(…)」쪽도 마찬가지
  const twin2 = indexIpgyeol(doc([R('종합', '교과(일반전형)', '간호학과', 3.6),
    R('교과', '교과(일반전형)', '간호학과', 3.6)]));
  eq(twin2.byKey.get(key('가대', '간호학과'))[0].cat, '교과', '「교과(…)」면 교과');

  // **학과가 다르면 지우지 않는다.** normDept 가 한 키로 합쳐 놓았을 뿐이다.
  const two = indexIpgyeol(doc([R('교과', '교과(지역인재)', '국제학부(일본학전공)', null),
    R('교과', '교과(지역인재)', '국제학부(중국학전공)', null)]));
  eq(two.byKey.get(key('가대', '국제학부(일본학전공)')).length, 2,
    '이름이 다른 학과는 둘 다 남긴다 — 지우면 없는 학과가 된다');

  // 공백만 다른 것은 같은 학과다
  const sp = indexIpgyeol(doc([R('종합', '종합(미래인재)', '스크랜튼학부 자유전공', 2.49),
    R('종합', '종합(미래인재)', '스크랜튼학부자유전공', 2.49)]));
  eq(sp.byKey.get(key('가대', '스크랜튼학부자유전공')).length, 1, '공백 차이는 같은 학과');
}
...
console.log('이름이 바뀐 전형 잇기');
{
  ...
  // 쪼개진 가지 **하나만** 이어지는 해에 남으면 1:1 로 보인다. 번호를 보고 막는다.
  eq(names(typeGroups([
    r(2025, '종합(학생부종합)', '종합'), r(2026, '종합(학생부종합Ⅰ)', '종합'),
  ])).length, 2, '이름 끝에 번호가 붙은 것은 이름이 바뀐 게 아니다');
  eq(names(typeGroups([
    r(2025, '교과(지역학생)', '교과'), r(2026, '교과(지역1)', '교과'),
  ])).length, 2, '아라비아 숫자도 마찬가지');
  // 다만 번호가 아니라 말이 바뀐 것은 그대로 잇는다
  eq(names(typeGroups([
    r(2025, '교과(추천)', '교과'), r(2026, '교과(학교장추천)', '교과'),
  ])).length, 1, '말이 바뀐 것은 잇는다');
...
console.log('지원한 전형의 입결 줄 고르기');
{
  ...
  /*
   * **작년에 이미 없어진 전형은 후보가 아니다** — 송원대 간호가 이 자리다.
   * 「교과(학생부우수) 2022~2025 → 교과(인성우수) 2026」이 한 묶음으로 이어지고,
   * 「교과(지역인재)」는 2024에 4명 뽑고 끝났다. 2027 모집요강의
   * 「학생부교과(면접우수자면접전형)」은 이름으로는 어느 쪽과도 안 겹친다.
   */
  const songwon = [
    r(2022, '교과(학생부우수)', 5.75), r(2023, '교과(학생부우수)', 5.75),
    r(2024, '교과(학생부우수)', 5.50), r(2025, '교과(학생부우수)', 6.25),
    r(2026, '교과(인성우수)', 5.60),
    r(2024, '교과(지역인재)', 5.40),
  ];
  ...
  // **둘 다 살아 있으면 아무 말도 안 한다.** 위의 `amb` 가 그 경우다(둘 다 2026).
  eq(e.fit, 'none', '마지막 해까지 둘 다 이어져 있으면 이 규칙은 안 나선다');

  // 면접·서류가 어긋나 지운 자리는 이 규칙으로도 안 메운다
  const markGap = [
    r(2024, '교과(지역인재)', 5.4), r(2026, '교과(서류우수)', 4.1),
  ];
  const mg = pickIpgyeol(markGap,

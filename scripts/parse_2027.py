...
정제 규칙:
  ...
  - '졸업제한' 컬럼을 새로 만든다. 지원자격 본문을 읽어 재수생 지원 가능 여부를
    '가능'/'현역만'/'확인필요' 로 판정한다 (자격학년도 컬럼은 뜻이 일정하지 않아 쓰지 않는다)
  - 완전 동일 중복행(NO 제외) 제거
...
# ── 졸업연도 제한 판정 ───────────────────────────────────────────────
# '자격학년도' 컬럼은 지원자격 본문에 나온 연도를 옮겨 적은 것일 뿐, 뜻이 일정하지 않다.
# 예) 자격학년도 2027 인 501행 가운데 258행의 본문은 오히려 "2027년 2월 이전 졸업예정자 포함"
#     이라서 재수생도 지원할 수 있다. 그래서 연도 대신 지원자격 본문으로 판정한다.
# 잘못 알려주면 지원할 수 있는 곳을 포기하게 되므로, 단정할 수 없으면 '확인필요'로 남긴다.
GRAD_OK = re.compile(
    r"졸업\(예정\)|졸업자|졸업\s*또는|이전\s*졸업|이전에\s*졸업"
    r"|동등\s*학력|동등한\s*학력|동등\s*이상의\s*학력|검정고시")
ONLY_NOW = re.compile(r"졸업예정자|3학년\s*재학|재학생")
AFTER_YR = re.compile(r"(20\d{2})년\s*\d?월?\s*이후\s*졸업")


def grad_limit(text):
    """지원자격 본문 → '가능'(졸업자도 지원 가능) / '현역만' / '확인필요'."""
    if not text:
        return "확인필요"
    m = AFTER_YR.search(text)
    if m:                                   # "N년 이후 졸업자" = 하한이 분명한 경우
        return "가능" if int(m.group(1)) <= 2026 else "현역만"
    if GRAD_OK.search(text):
        return "가능"
    if ONLY_NOW.search(text):
        return "현역만"
    return "확인필요"
...
def main():
    ...
    out_cols = [c for c in COLS if c != "NO"] + ["특이표기", "졸업제한"]
    rows = []
    for _, r in df.iterrows():
        mark = new_mark(r)
        glim = grad_limit(clean(r["지원자격"]))
        row = []
        for c in out_cols:
            v = mark if c == "특이표기" else glim if c == "졸업제한" else clean(r[c])
            if v is None:
                row.append(None)
            elif c in NUM_COLS:

...
병합 규칙
  ...
  4) 경쟁률이 1 미만이면서 엑셀 시간서식으로 오파싱된 흔적이 있고,
     복원값이 엑셀의 다른 행과 일치할 때만 되돌린다.
     흔적은 (a) 값이 정확히 분 단위로 떨어지고 (b) 소수 두 자리로 표현되지 않는 것.
     경쟁률은 보통 두 자리까지만 쓰므로 0.5·0.25 같은 값은 실제 미달로 보고 건드리지 않는다.
"""
...
def timeserial(v):
    ...
    if abs(round(v, 2) - v) < 1e-9:
        return None                     # 0.5·0.25 처럼 두 자리로 쓸 수 있으면 실제 미달
    m = int(round(v * 1440))
    return m // 60 + (m % 60) / 100


def fix_comp(u, y, dept, v):
    """0.375694 처럼 '9:01'로 읽힌 값만 9.01 로 되돌린다."""
